...
func PrintNotifier(ctx context.Context, duration int64) {
	...
	for {
		...
		case <-ticker.C:
			...
			l3Table.SetHeader([]string{"Index", "SrcAddr", "DstAddr", "DstLocation",
			...
			for _, f := range fc.L3FlowMap {
				...
				m := []string{
					...
					f.SrcAddr,
					f.DstAddr,
					fmt.Sprintf("%s, %s", f.DstCountry, f.DstCity),
			...
			if config.IsDecodeL4 {
				...
				l4Table.SetHeader([]string{"Index", "SrcAddr", "DstAddr", "DstLocation", "SrcPort", "DstPort", "Protocol",
						"BytesIn", "PacketsIn", "DurationIn", "RateIn",
						"BytesOut", "PacketsOut", "DurationOut", "RateOut"})
